...
const NOTEBOOK_RF_PATH = path.join(__dirname, 'model', 'rf-model.json');
...
/* ============================================================
   NOTEBOOK RANDOM-FOREST INFERENCE

   When the RandomForest exported from RealStatePricePrediction.ipynb is
   present, it is preferred. The notebook trains on log(price), so the tree
   average is exponentiated back to BDT. The JSON export keeps inference
   native to Node and avoids adding a Python runtime to the web server.
============================================================ */

let notebookRFCache;

function loadNotebookRF() {
    if (notebookRFCache !== undefined) return notebookRFCache;
    try {
        notebookRFCache = fs.existsSync(NOTEBOOK_RF_PATH)
            ? JSON.parse(fs.readFileSync(NOTEBOOK_RF_PATH, 'utf8'))
            : null;
    } catch (error) {
        console.warn('⚠ Could not load exported notebook RF model:', error.message);
        notebookRFCache = null;
    }
    return notebookRFCache;
}

function encodeNotebookValue(model, column, value) {
    const classes = model?.encoders?.[column];
    if (!Array.isArray(classes)) return 0;
    const text = String(value ?? '');
    const index = classes.findIndex(v => String(v) === text);
    return index >= 0 ? index : 0;
}

function notebookFeatureInput(input, model) {
    const amenities = Array.isArray(input.amenities)
        ? input.amenities.length
        : Object.values(input.amenities || {}).filter(Boolean).length;

    const datasetValues = {
        area: toSqft(input.area, input.areaUnit),
        building_type: input.propertyType || 'Apartment',
        building_nature: input.constructionStatus || '',
        image_url: '',
        num_bath_rooms: Number(input.bathrooms) || 0,
        num_bed_rooms: Number(input.bedrooms) || 0,
        property_description: input.description || '',
        property_overview: input.description || '',
        property_url: '',
        purpose: input.purpose || 'Rent',
        city: input.city || '',
        locality: input.areaName || '',
        ...
        unclassify_amenity_count: amenities,
        division: input.district || '',
        zone: input.areaName || ''
    };

    return (model.featureCols || []).map(column => {
        const value = datasetValues[column];
        return model.encoders && model.encoders[column]
            ? encodeNotebookValue(model, column, value)
            : Number.isFinite(Number(value)) ? Number(value) : 0;
    });
}

function predictWithNotebookRF(input) {
    const model = loadNotebookRF();
    if (!model || !Array.isArray(model.trees) || !model.trees.length) return null;

    const features = notebookFeatureInput(input, model);

    function predictTree(tree) {
        let node = 0;
    ...
    const logPrediction = model.trees.reduce((sum, tree) => sum + predictTree(tree), 0) / model.trees.length;
    const estimate = Math.max(0, Math.round(Math.expm1(logPrediction)));
    if (!estimate) return null;

    const purpose = String(input.purpose || 'Rent').toLowerCase().includes('sale') ? 'Sale' : 'Rent';
    const areaSqft = Math.round(toSqft(input.area, input.areaUnit));
    const rate = areaSqft > 0 ? Math.round(estimate / areaSqft) : 0;
    const spread = Math.max(Math.round(estimate * 0.12), 5000);
    ...
    return {
        ...
        modelType: 'RealStatePricePrediction.ipynb RandomForestRegressor',
        modelAccuracy: 'R² 87.51% on the notebook test split',
        explanation: 'Prediction generated from the RandomForestRegressor trained in RealStatePricePrediction.ipynb. The trained model predicts log-transformed property price and the result is converted back to BDT.'
    };
}
...
function predictPrice(input) {
    ...
    const notebookPrediction = predictWithNotebookRF(input);
    if (notebookPrediction) return notebookPrediction;
...
module.exports = {
    ...
    AMENITY_COLUMNS,
    FEATURE_NAMES,
    hasNotebookRFModel: () => Boolean(loadNotebookRF()),
    predictWithNotebookRF
};
