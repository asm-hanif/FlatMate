...
async function sendPropertyReportEmail({
    ownerEmail,
    ownerName,
    websiteOwnerEmail,
    reporterName,
    ...
    const recipients = [ownerEmail, websiteOwnerEmail]
        .map(value => String(value || '').trim())
        .filter((value, index, list) => value && list.indexOf(value) === index);

    if (!recipients.length) {
        throw new Error('No report recipient email address is configured.');
    }

    const safeOwnerName = escapeHtml(ownerName || 'Property Owner');
    const safeReporterName = escapeHtml(reporterName || 'FlatMate user');
    const safeReporterEmail = escapeHtml(reporterEmail || 'Not provided');
    const safeReporterPhone = escapeHtml(reporterPhone || 'Not provided');
    const safeFlatTitle = escapeHtml(flatTitle || `Property #${flatId}`);
    const safeReason = escapeHtml(reason);
    const safeDetails = escapeHtml(details || 'No additional details were provided.');
    const propertyUrl = `${process.env.APP_BASE_URL || `http://localhost:${process.env.PORT || 3000}`}/flat.html?id=${encodeURIComponent(flatId)}`;

    return sendMail({
        to: recipients.join(', '),
        subject: `[FlatMate Report] ${flatTitle || `Property #${flatId}`}`,
        text: `
FlatMate Property Report

A property has been reported and requires attention.

Property: ${flatTitle || `Property #${flatId}`}
Property ID: ${flatId}
...
Reporter:
Name: ${reporterName || 'FlatMate user'}
...
Property owner:
${ownerName || 'Property Owner'} — ${ownerEmail || 'No email provided'}

This report was submitted through the FlatMate website.
        `.trim(),
        html: `
<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"><title>FlatMate Property Report</title></head>
<body style="margin:0;padding:30px;background:#f5f4ef;font-family:Arial,sans-serif;color:#292a26;">
<div style="max-width:680px;margin:auto;background:#fff;border:1px solid #deddd7;border-radius:12px;overflow:hidden;">
    <div style="padding:25px 30px;background:#292a26;color:#fff;">
        <div style="font-size:24px;font-weight:bold;letter-spacing:2px;">FLATMATE</div>
        <div style="margin-top:6px;font-size:11px;letter-spacing:2px;opacity:.75;">PROPERTY &amp; LIVING</div>
    </div>
    <div style="padding:30px;">
        <h2 style="margin:0 0 8px;font-family:Georgia,serif;font-weight:500;">Property Report</h2>
        <p style="line-height:1.7;margin-top:0;">A FlatMate property has been reported and requires attention.</p>

        <div style="margin:22px 0;padding:18px;background:#f5f4ef;border-radius:8px;">
            <div style="font-size:12px;color:#777;text-transform:uppercase;letter-spacing:1px;">Property</div>
            <div style="font-size:18px;font-weight:bold;margin-top:6px;">${safeFlatTitle}</div>
            <div style="margin-top:6px;color:#777;">Property ID: ${flatId}</div>
            <a href="${escapeHtml(propertyUrl)}" style="display:inline-block;margin-top:14px;color:#1f5c4d;">View property</a>
        </div>

        <table style="width:100%;border-collapse:collapse;margin-bottom:20px;">
            <tr><td style="padding:9px 0;font-weight:bold;width:150px;">Reason</td><td style="padding:9px 0;">${safeReason}</td></tr>
            <tr><td style="padding:9px 0;font-weight:bold;">Reporter</td><td style="padding:9px 0;">${safeReporterName}</td></tr>
            <tr><td style="padding:9px 0;font-weight:bold;">Email</td><td style="padding:9px 0;">${safeReporterEmail}</td></tr>
            <tr><td style="padding:9px 0;font-weight:bold;">Phone</td><td style="padding:9px 0;">${safeReporterPhone}</td></tr>
            <tr><td style="padding:9px 0;font-weight:bold;">Owner</td><td style="padding:9px 0;">${safeOwnerName}</td></tr>
        </table>

        <div style="padding:20px;background:#f5f4ef;border-radius:8px;line-height:1.7;white-space:pre-wrap;">
            <strong>Additional details</strong>
            <div style="margin-top:8px;">${safeDetails}</div>
        </div>
    </div>
    <div style="padding:18px 30px;border-top:1px solid #e5e4df;color:#77776f;font-size:12px;">
        This report was submitted through the FlatMate website.
    </div>
</div>
</body>
