...
/* POST /api/bot/chat — optional natural-language LLM chat for Mira. */
async function chat(req, res) {
    try {
        const messages = Array.isArray(req.body.messages) ? req.body.messages : [];
        const text = await llmService.chat({
            messages,
            propertyContext: req.body.propertyContext || null
        });

        return res.json({
            success: true,
            llm: Boolean(text),
            message: text || 'I can still help with FlatMate using my built-in property assistant. Ask me about pricing or finding a home.'
        });
    } catch (error) {
        console.error('Bot LLM chat error:', error.message);
        return res.status(200).json({
            success: true,
            llm: false,
            message: "I'm having trouble reaching the conversational AI right now. I can still help with property pricing and home search."
        });
    }
}

module.exports = { getMeta, priceSuggest, findFlats, chat };
